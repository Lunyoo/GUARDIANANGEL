...
class BotVigia {
  ...
  private async getProductContext(): Promise<string> {
    try {
      const productInfo = await getProductInfo('calcinha-modeladora-default')
  ...
  private async validateCityInfo(userMessage: string, botResponse: string): Promise<AnalysisResult> {
    ...
    if (needsClarification) {
      return {
        ...
        suggestedResponse: 'Para o FRETE GRÁTIS e entrega correta, preciso saber a cidade específica. Qual cidade exatamente?',
        severity: 'medium'
      }
    }

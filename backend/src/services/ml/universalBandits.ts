...
export class UniversalBanditSystem {
  ...
  private initializeArms() {
    ...
    // 🗣️ BANDITS DE ABORDAGEM (8 estilos diferentes)
    const approachArms = [
      { id: 'approach_direct', variant: 'Direto aos benefícios', context: { style: 'direct', script: 'Oi! A calcinha lipo modela na hora, levanta bumbum e afina cintura! Qual tamanho você usa?' } },
    ...
    // ⏰ BANDITS DE TIMING (6 intervalos diferentes)
    const timingArms = [
      { id: 'timing_instant', variant: 'Resposta instantânea', context: { delay: 0 } },
      { id: 'timing_quick', variant: 'Resposta rápida', context: { delay: 500 } },
      ...
      { id: 'timing_strategic', variant: 'Estratégico', context: { delay: 6000 } }
    ];
  ...
  { id: 'closing_risk_reversal', variant: 'Reversão risco', context: { technique: 'risk_reversal', script: 'Sem risco: te ajudo a escolher o tamanho certinho. Por higiene, não fazemos trocas.' } },
      { id: 'closing_scarcity', variant: 'Escassez', context: { technique: 'scarcity', script: 'Só restam poucas no seu tamanho. Garante a sua?' } },
  { id: 'closing_trial', variant: 'Teste', context: { technique: 'trial', script: 'Que tal começar com 1 unidade hoje para sentir o efeito? 😉' } },
  ...
  { id: 'script_size_doubt', variant: 'Dúvida tamanho', context: { situation: 'size_doubt', script: 'Sem problemas! A calcinha tem elastano. Eu te ajudo a acertar o tamanho. Por higiene, não fazemos trocas.' } },
    ...
    const objectionArms = [
      ...
      { id: 'objection_promo_3por1_encerrada', variant: 'Promoção 3 por 1 encerrada', context: { 
        type: 'promotion_ended', 
        trigger: ['3 por 1', 'anúncio dizia', 'vi que era 3'],
        response: 'Ai que pena! Essa promoção já encerrou ontem 😔 Mas posso fazer um desconto especial pra você: 2 unidades por R$ 119,90! O que acha?'
      }}
    ]
  ...
  private filterRelevantArms(arms: UniversalBanditArm[], context: BanditContext): UniversalBanditArm[] {
    const filtered = arms.filter(arm => {
      ...
      if (arm.category === 'pricing') {
        ...
        return true // Se não tem quantidade específica, incluir todos
      }
  ...
  updateDeliveryContext(deliveryConfig: any) {
    try {
      ...
      console.log(`🚚 Contexto de entrega atualizado: ${deliveryConfig.codCities?.length || 0} cidades COD`)
      return true
    } catch (error) {
  ...
  addArm(arm: UniversalBanditArm) {
    ...
    // this.persist() - método não existe, removido
    console.log(`✅ Adicionado novo arm: ${arm.variant} (categoria: ${arm.category})`)
  }
  ...
  resetArm(armId: string) {
    ...
    if (arm) {
      ...
      arm.ucbScore = 1.0
      // this.persist() - método não existe, removido
      console.log(`🔄 Reset arm: ${arm.variant}`)
    }
  }
  ...
  increaseExploration(category: UniversalBanditArm['category'], amount: number) {
    ...
    categoryArms.forEach((arm: UniversalBanditArm) => {
      arm.ucbScore += amount // Aumenta UCB para forçar exploração
    })
    console.log(`🔍 Aumentou exploração da categoria ${category} em ${amount}`)
  }
